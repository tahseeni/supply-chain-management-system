...
public class OrderForm {
	...
	/**
	 * OrderForm constructor to intilialize InventoryHandler object and create order
	 * object
	 */
	...
	/**
	 * Method to set orderCombinations equal an ArrayList containing all valid
	 * combinations of furniture items
	 * 
	 * @param orderCombos - ArrayList containing valid combinations of furniture
	 *                    items (contain all required parts)
	 */
	...
	/**
	 * Method to set order equal to an ArrayList containing all furniture items from
	 * the cheapest combination
	 * 
	 * @param index       - index of cheapest combination of furniture items
	 * @param orderCombos - ArrayList containing all valid combinations of furniture
	 *                    items (contain all required parts)
	 */
	 ...
	 * 
	 * @param inv - InventoryHandler object containing combinations of furniture
	 ...
	 * Prints messages to the console for the user, confirming that the order form
	 * has been generated, or that it is unable to fulfill the order. The method
	 * will print the suggested manufacturers if the order is invalid. No return
	 * value.
	 * 
	...
	public void createOrderForm(SQLConnector db, int userQty, String userType, String userItem) {
		...
		if (!this.getOrderCombinations().isEmpty()) {
			System.out.println("The order is valid. It will now be processed.");
			this.setOrder(this.inventory.findCheapest(this.getOrderCombinations()), this.getOrderCombinations());

			// generate order for the terminal and the output file
	...
	/**
	 * Method to generate order receipt, using FileWriter objects. Called by
	 * createOrderForm(), no return value.
	 * 
	...
	/**
	 * Method to display order details via the terminal
	 */
	public void displayOrder() {
		System.out.print("Output: Purchase ");
		for (int i = 0; i < this.getOrder().size(); i++) {
			if (i != this.getOrder().size() - 1) {
				System.out.print(this.getOrder().get(i).getID() + ", ");
			} else {
				System.out.print("and " + this.getOrder().get(i).getID() + " for $"
						+ this.calculateOrderTotal(this.getOrder()) + ".\n");
			}
		}
	}
